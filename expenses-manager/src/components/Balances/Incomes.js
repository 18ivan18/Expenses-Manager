...
const Incomes = ({ date }) => {
  ...
  const categories = useSelector((state) => state.balance.categorys);
  ...
  useEffect(() => {
    if (auth.loggedIn) {
      ...
      dispatch(getIncomes(auth)).then(() => dispatch(changeLoading()));
    }
  }, [auth, message]);
    ...
    <div>
      ...
      <div className="col s6">
        <div style={{ textAlign: "center" }}>
          <h3>{`Incomes for ${parseDate1(date.toString())}`}</h3>
        </div>
        {incomesForDay &&
